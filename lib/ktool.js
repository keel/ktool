...
/**
 * Base64算法,用于生成递增短string,如:Base62.encode(new Date().getTime());
 * @type {Object}
 */
const Base62 = {
  ...
  'instance': function() {
    ...
    n.arr = shuffle(['3', 'd', 'h', 'i', 'a', 's', 'c', 'k', 'o', 'R', 'Z', 'w', 'u', '7', 'P', '5', '0', 'L', '6', 'S', '4', 'I', 'X', 'Q', 'j', 'G', 'x', 'W', 'F', 'E', 'e', 'M', 'f', 'p', '2', 'l', 'g', 'q', 'D', 'U', 'n', 'v', 'B', 'b', 'O', 'K', 'T', 'N', 'V', 'y', 'Y', '9', 'm', 'z', '1', 'H', 'C', 'A', '8', 't', 'r', 'J']);
    return n;
  },
...
//对json遍历操作，注意默认不会对key执行fn
const jsonFilter = function jsonFilter(json, fn, isIncludeKey) {
  if (Array.isArray(json)) {
    for (let i = 0, len = json.length; i < len; i++) {
      json[i] = jsonFilter(json[i], fn);
    }
    return json;
  }
