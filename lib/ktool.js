...
(async function() {
  ...
  await ktool.promi(ktool.sleep)(2000);
...
使用2(==setTimeout):
ktool.sleep(1000,runFn);
 */
const sleep = function(sleepMS, callback) {
  setTimeout(function() {
    callback();
  }, sleepMS);
};
...
const objToParas = function(reqObj) {
  let out = '';
  for (const i in reqObj) {
    out += '&' + i + '=' + reqObj[i];
  }
  if (out.length > 0) {
    out = out.substring(1);
...
const parseCookie = function(cookieStr) {
  if (!cookieStr) {
    return null;
  }
  const out = {};
  const cArr = cookieStr.split(';');
  for (let i = 0, len = cArr.length; i < len; i++) {
    const one = cArr[i];
    const oArr = one.split('=');
    if (oArr.length >= 2) {
      out[oArr[0].trim()] = oArr[1].trim();
    }
  }
  return out;
};

//这里指获取新发送的cookie,即set-cookie
...
const getCookiePair = function(cookieStr, cookieMap) {
  if (!cookieMap) {
    cookieMap = {};
  }
  const splitPo = cookieStr.indexOf('=');
  if (splitPo > 0) {
    const key = cookieStr.substring(0, splitPo).trim();
    const value = cookieStr.substring(splitPo + 1).trim();
    cookieMap[key] = value;
  }
  return cookieMap;
};
...
const updateCookie = function(orgCookie, headers) {
  ...
  for (let i = 0, len = orgArr.length; i < len; i++) {
    getCookiePair(orgArr[i], cooMap);
  }
  // console.log('cooMap:%j',cooMap);
  const cooArr = headers['set-cookie'];
  for (let i = 0, len = cooArr.length; i < len; i++) {
    getCookiePair(cooArr[i].split(';')[0], cooMap);
  }
  // console.log('cooMap2:%j',cooMap);
  let cookie = '';
  Object.keys(cooMap).forEach((i) => {
    cookie += i + '=' + cooMap[i] + '; ';
  });
  return cookie;
};
...
// console.log(parseCookie(orgCookie));
// const headers = {
//   'set-cookie': [
 ...
 */
const strToArr = function(inputStr) {
...
const jsonFilter = function(json, fn, key = null, isIncludeKey = false) {
 ...
 */
const merge = function(orgObj, newVal) {
...
const xssFilter = function(bodyJson) {
  if (typeof bodyJson === 'string') {
    return XSSFilter.xssFilterFn(bodyJson);
  }
  return jsonFilter(bodyJson, XSSFilter.xssFilterFn);
};
const xssFilterBack = function(bodyJson) {
  if (typeof bodyJson === 'string') {
    return XSSFilter.xssFilterBackFn(bodyJson);
  }
...
exports.randomDraw = random.randomDraw;
...
exports.sha256 = str.sha256;
exports.hexStr = str.hexStr;
exports.hmacSha1Base64 = str.hmacSha1Base64;
exports.hmacSha1 = str.hmacSha1;
exports.hmacSha256Base64 = str.hmacSha256Base64;
exports.hmacSha256 = str.hmacSha256;
exports.httpPost = net.httpPost;
exports.httpReq = net.httpReq;
...
exports.createApiServer = net.createApiServer;
...
exports.readLineTxt = IO.readLineTxt;
exports.sleep = sleep;
exports.SM4 = SM4;
exports.objToParas = objToParas;


const filterProxy = require('./filterProxy');
exports.filterProxy = filterProxy;

const stdinPlus = require('./stdinPlus');
exports.stdinPlus = stdinPlus;