...
var getSend = function(type, opts, options, callback) {
  var getType = (type === 'https') ? https : http;
  var encode = 'utf8';
  if (cck.check(options, 'function')) {
    //options参数不存在,直接认为是callback
    callback = options;
...
var httpGet = function(reqUrl, options, callback) {
  var opts = url.parse(reqUrl);
  getSend('http', opts, options, callback);
};


var httpsGet = function(reqUrl, options, callback) {
  var opts = url.parse(reqUrl);
