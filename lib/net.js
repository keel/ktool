...
const httpSend = function(opts, data, options, callback) {
  ...
  let timeout = 0;
  let redirTimes = 0; //重定向次数
  if (cck.check(options, 'function')) {
    //options参数为function,直接认为是callback
    callback = options;
  } else {
    if (options.redirTimes) {
      redirTimes = options.redirTimes || 0;
      delete options.redirTimes;
    }
  ...
  const req = httpType.request(opts, function(res) {
    ...
    if (redirTimes && res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307 || res.statusCode === 308) {
      //重定向
      const newUrl = res.headers.location;
      if (newUrl) {
        const newCookieArr = res.headers['set-cookie'] || null;
        res.destroy();
        req.destroy();
        redirTimes--;
        if (redirTimes > 0) {
          options.redirTimes = redirTimes;
          if (newCookieArr) {
            const newCookie = newCookieArr[0];
            if (!options.headers) {
              options.headers = {};
            }
            //处理set-cookie
          ...
          // console.log('=====>newUrl:',newUrl, 'resHeaders', res.headers, 'options',options);
          httpSend(mkOpts(newUrl, opts.method), data, options, callback);
          return;
        } else {
          outError = new Error('重定向超过次数', redirTimes);
        }
      } else {
        outError = new Error('重定向响应缺少Location头');
      }
    }
    ...
    onDataTarget.on('data', function(chunk) {
      bodyBuf = Buffer.concat([bodyBuf, chunk]);
    });
    onDataTarget.on('end', function() {
      if (outError) {
