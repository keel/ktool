...
const postSend = function(type, opts, data, options, callback) {
  ...
  const req = postType.request(opts, function(res) {
    ...
    if (res.statusCode !== 200) {
      ...
      getError = new Error(type + '-POST[' + opts.href + '] statusCode:[' + res.statusCode + ']');
    }
    let bodyBuf = Buffer.from('');
    res.on('data', function(chunk) {
      bodyBuf = Buffer.concat([bodyBuf, chunk]);
...
const getSend = function(type, opts, options, callback) {
  ...
  getType.get(opts, function(res) {
    if (res.statusCode !== 200) {
      ...
      // return callback(new Error(type + '-GET[' + opts.href + '] statusCode:[' + res.statusCode + ']'), res);
    }
    let bodyBuf = Buffer.from('');
    res.on('data', function(chunk) {
      bodyBuf = Buffer.concat([bodyBuf, chunk]);
...
// httpGet('https://ga.gametdd.com/tdCtrl/login', { 'encode': 'utf8' }, function(err, re, res) {
//   if (err) {
...
// httpsPost('https://ga.gametdd.com/adCtrl/pay_mi', '{"test":"ok"}', { 'headers': { 'Content-Type': 'application/json' } }, function(err, re, res) {
//   if (err) {
//     console.error(err.stack);
//     console.log('' + re);
//     console.log('statusCode:' + res.statusCode);
