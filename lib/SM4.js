/**
 * 国密sm4加密,这里是ecb/pkcs7的实现,使用hex转string
 *
 *
 * 国密即国家密码局认定的国产密码算法。
 ...
 * const enc = SM4.create('key').encryptData_ECB('ABC');
 * const dec = SM4.create('key').decryptData_ECB(enc);
 *
 */
...
function instance(key) {
  ...
  function decryptData_ECB(cipherText) {
    try {
      ...
      const decrypted = sm4_crypt_ecb(ctx, Buffer.from(cipherText, 'hex'));
...
// const SM4 = require('../../lib/SM4');
// const sKey = 'qimht1gmcqcmi34h';
// const sm4 = SM4.create(sKey);
// const enc = sm4.encryptData_ECB('ABC');
// const dec = sm4.decryptData_ECB(enc);
// console.log(enc);
// console.log(dec);
