...
const proxy = function(req, res, localUrlPath) {
  const remoteUrl = new URL(remoteHost);
  const options = {
    'host': remoteUrl.host,
    'port': remoteUrl.port || '80',
    'path': Path.join(remoteUrl.path, localUrlPath),
    'method': req.method,
...
const webSvr = function(req, res) {
  ...
  // llog('===> REQ: ' + reqUrl);
  let pathName = (reqUrl.startsWith('http')) ? new URL(reqUrl).pathname : reqUrl;
  ...
  if (req.method !== 'GET' && remoteHost !== 'NO') {
    //对非GET请求转发到远程服务器
...
// start(null, '/Users/keel/dev/workspace/h5game/app/src/main/assets/web/', 'NO');
