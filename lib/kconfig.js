 ...
 * @return {}
 */
const reInit = function(isForce = false, localConfigFile = null, envConfigFileName = null, noDefaultConfigName = 'default', callback) {
  if (isInitStart) {
    if (callback) {
  ...
  if (configMap[noDefaultConfigName] && !isForce) {
  ...
  if (noDefaultConfigName === 'default') {
    configObj = newConfigObj;
  }
  configMap[noDefaultConfigName] = newConfigObj;
  isInitStart = false;
  vlog.log('kconfig init OK! \n------\n%s\n------', newConfigObj.toString());
...
const readEncConfigFile = function readEncConfigFile(filePath) {
  try {
    ...
    if (!configStr) {
      vlog.error('readEncConfigFile,read config file failed.', filePath);
      return null;
    }
    ...
    if (!json) {
      vlog.error('readEncConfigFile,configStrToJson failed:%j', filePath);
      return null;
    }
    return json;
  } catch (e) {
    vlog.eo(e, 'readEncConfigFile,config file is not exist or not json.', filePath);
    return null;
  }
};
...
const loadEnvConfig = function loadEnvConfig(localConfigObj, envDir, envConfigFileName) {
  ...
  if (!fs.statSync(envFile).isFile()) {
    vlog.error('===== > env config file is not found!!', envFile);
    return thisConfig;
  }
 ...
 * @param  {string} noDefaultConfigName
 * @return {anyType}
 */
const get = function(jsonPath, noDefaultConfigName = 'default') {
  const configTaget = configMap[noDefaultConfigName];
  if (!configTaget) {
    vlog.error('get config failed, need init.', jsonPath, noDefaultConfigName);
    return null;
  }
